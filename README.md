...
- In-app diagnostics page for app logs, trip calculation logs, successful and failed web-login
  ...
  under the App Log.
- Authenticated desktop navigation uses the same color-coded button identity as the mobile web-app
  layout, where navigation becomes icon-only in one full-width top-bar row and leaves the bottom
  safe area clear for phone system navigation without opting into edge-to-edge phone drawing. The
  login page does not show the shared top navigation.
